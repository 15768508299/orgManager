    ...
    function deleteMes(){
        $.messager.confirm('确认','确定要删除这些数据，删除数据不可恢复(慎重!)',function(r){
            if (r){
                ...
                $.ajax({
                    url:'<%=basePath%>/org_news/deleteOL?ids='+ids,
...
<div class="myDiv">
    ...
    <div id="tb" style="padding:5px;height:auto">
        <div style="margin-bottom:5px">
            <a href="javascript:void(0)" onclick="addMes('<%=basePath%>org_news/newsOlAdd','新增轮播信息')" class="easyui-linkbutton" iconCls="icon-add" plain="true">新增轮播信息</a>
            <a href="javascript:void(0)" onclick="editMes('<%=basePath%>org_news/newsOlAddEdit','编辑轮播信息')" class="easyui-linkbutton" iconCls="icon-edit" plain="true">编辑轮播信息</a>
            <a href="javascript:void(0)" onclick="deleteMes()" class="easyui-linkbutton" iconCls="icon-cut" plain="true">删除轮播信息(可多选)</a>
        </div>
        <div>
            轮播描述: <input class="easyui-textbox" id="search" style="width:160px">
...
<script>
    $(function () {
        $('#dg').datagrid({
            method: "POST",
            url:"<%=basePath%>org_news/newOlList",
            pageSize: 10,//初始化分页大小
