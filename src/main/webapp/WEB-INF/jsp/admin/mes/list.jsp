...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="<%=basePath %>css/my/admin/mes/list.css">
    <script type="text/javascript" src="<%=basePath %>js/jquery.min.js"></script>
    <script type="text/javascript" src="<%=basePath %>js/jquery.easyui.min.js"></script>
    <script type="text/javascript" src="<%=basePath %>js/admin/mes/list.js"></script>
...
<script type="text/javascript">
    ...
    function editMes(url,title){
        ...
        if(row.length>1){
            ...
            console.log(newUrl);
            openWindow(1109,510,newUrl,title);
        }
    }
    function deleteMes(){
        $.messager.confirm('确认','确定要删除这些数据，删除数据不可恢复(慎重!)',function(r){
            if (r){
                ...
                $.ajax({
                    url:'<%=basePath%>/org_mes/delete?ids='+ids,
...
<div class="myDiv">
    ...
    <div id="tb" style="padding:5px;height:auto">
        <div style="margin-bottom:5px">
            <a href="javascript:void(0)" onclick="addMes('<%=basePath%>org_mes/add','新增社团信息')" class="easyui-linkbutton" iconCls="icon-add" plain="true">新增社团信息</a>
            <a href="javascript:void(0)" onclick="editMes('<%=basePath%>org_mes/edit','编辑社团信息')" class="easyui-linkbutton" iconCls="icon-edit" plain="true">编辑社团信息</a>
            <a href="javascript:void(0)" onclick="deleteMes()" class="easyui-linkbutton" iconCls="icon-cut" plain="true">删除社团信息(可多选)</a>
        </div>
        <div>
            社团名称: <input class="easyui-textbox" id="search" style="width:160px">
...
<script>
    $(function () {
        $('#dg').datagrid({
            method: "POST",
            url:"<%=basePath%>org_mes/adminGetList",
            pageSize: 10,//初始化分页大小
            ...
            columns:[[
                ...
                {field:'id',title:'ID',width:48},
                {field:'mesName',title:'社团名称',width:154,sortable:true},
                {field:'mesPhoto',title:'图片路径',width:90,formatter:toWatchButton},
                {field:'mesCreate',title:'社团创建人',width:85,sortable:true},
                {field:'mesCreateTime',title:'社团创建时间',width:110,sortable:true,formatter:timeStapmToString},
                {field:'mesDetail',title:'图文介绍',width:83,formatter:toWatchDetail},
            ]],
            toolbar: '#tb',
